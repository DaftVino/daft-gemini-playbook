...
# 922 — Attestation Does Not Apply To Prompts
...
The synthetic-data marker waives figures in worked examples. It must not waive them in a prompt: a
prompt has no legitimate use for a specific number, because the standard requires bracketed
placeholders and `$X`. The $12,345 below must still be reported.
